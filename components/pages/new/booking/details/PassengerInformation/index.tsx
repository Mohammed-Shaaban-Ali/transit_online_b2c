...
function PassengerInformation({ formData }: Props) {
  const t = useTranslations("BookingDetails.PassengerInformation");
      ...
      <div className="space-y-6">
        {passengers.map((passenger, index) => {
          const gender = passenger.gender
            ? t(passenger.gender === "female" ? "female" : "male")
            : t("male");
          const type = passenger.type
            ? t(passenger.type === "child" ? "child" : passenger.type === "infant" ? "infant" : "adult")
            : t("adult");
          const dateOfBirth = passenger.dateOfBirth
            ? new Date(passenger.dateOfBirth).toLocaleDateString(locale === "ar" ? "ar-EG" : "en-US", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })
          ...
          return (
            ...
            >
              ...
              <div className="grid grid-cols-1  gap-2 gap-x-5 text-14 text-gray-500 font-normal  leading-tight  md:grid-cols-[120px_1fr]">
                <p className=" ">{t("idType")}:</p>
                <p className="">{t("passport")}</p>

                <p className=" ">{t("idNumber")}:</p>
                <p className=" ">{passenger.passportNumber || t("notAvailable")}</p>

                <p className=" ">{t("nationality")}</p>
                <p className=" ">{passenger.nationality || t("notAvailable")}</p>
