...
} from "@/components/pages/new/home/HotelsOffers";

function HotelsOffersPage() {
  const t = useTranslations("NewPage.home.round");
  const locale = useLocale();
  const isRtl = locale === "ar";
  const [page, setPage] = useState(1);
  const [items, setItems] = useState<IHotelOffer[]>([]);
  const [isAppending, setIsAppending] = useState(false);
  const { data, isLoading, isFetching } = useGetHotelOffersQuery({
    page: page || 1,
  });
      ...
      <section className="container mx-auto w-full max-w-[1200px]! px-3 pb-8 pt-28 md:pt-24">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {items.map((offer) => (
                <HotelOfferCard offer={offer} key={offer.id} isRtl={isRtl} />
              ))}
            </div>
...
export default HotelsOffersPage;
